...
# Modifications copyright (C) 2026 - Scott Smart <scott967@kodi.tv>
# This program is Free Software see LICENSE file for details
"""Main class object run when called by Runscript() as executable in Kodi
Note:  Endpoints from Trakt are no longer supported.  If called with
info=trending, info=popular, info=anticipated, info=collected, info=watched,
info=recommended or info=watchlist, the request will be ignored/logged and no data will be returned.
"""
...
TRAKT_ENDPOINTS = ["trending", "popular", "anticipated",
                   "collected", "watched", "recommended", "watchlist"]
...
class Main:
    ...
    def __init__(self):
        """Initializes the main process as object when extendedinfo is called as script
        ...
        from TMDB retrieved by popularity.   TRAKT endpoints no longer supported.
        """
        ...
        utils.log(
            f'default.Main setting LanguageIDv2 after update {addon.setting("LanguageIDv2")}', adb=True)
        self._parse_argv()
    ...
    def _parse_argv(self) -> None:
        ...
        for arg in sys.argv[1:]:
            ...
            if param.startswith('info='):
                if param[5:] in TRAKT_ENDPOINTS:
                    utils.log(
                        f'default._parse_argv info= {param[5:]} is a TRAKT endpoint: ignored', adb=True)
                else:
                ...
                try:
                    self.params[param.split('=', maxsplit=1)[0].lower()] = "=".join(
                        param.split("=")[1:]).strip()
                    utils.log(
                        f'default._parse_argv self.params {self.params}', adb=True)
                except Exception:
                    pass
        utils.log(
            f'default._parse_argv final self.infos {self.infos} self.params {self.params}')
